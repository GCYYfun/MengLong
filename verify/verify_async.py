...
async def test_menglong_async():
    ...
    model = Model(
        default_model_id="anthropic/global.anthropic.claude-sonnet-4-5-20250929-v1:0"
    )

    # 测试异步聊天
    print("\n1. 测试 async_chat:")
    try:
        response = await model.async_chat([User("你好，请简单介绍一下你自己。")])
...
async def test_concurrent_requests():
    ...
    # 并发执行多个请求
    prompts = ["What is 2+2?", "What is the capital of France?", "What is Python?"]
