...
class DemoAgent:
    def analyze_requirements(self, task: str) -> dict:
        print(f"\n🔍 正在分析需求: {task}")
        time.sleep(1)
        return {"framework": "FastAPI", "database": "PostgreSQL", "auth": "JWT"}
...
if __name__ == "__main__":
    ...
    workflow = Workflow(
        config_file=os.path.join(os.path.dirname(__file__), "workflow_config.yaml")
    )
    ...
    print("🚀 开始执行工作流...")
    result = workflow.execute_workflow(agent=agent, debug=True)
