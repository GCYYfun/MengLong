...
class ThinkingPart(ContentPart):
    ...
    type: str = "thinking"
    thinking: str
    signature: Optional[str] = None  # Anthropic extended thinking 需要回传
...
def Assistant(
    content: Union[str, "Response", None] = None,
    actions: Optional[Sequence[Union[Dict[str, Any], Action]]] = None,
    reasoning: Optional[str] = None,
) -> Message:
    """快捷构造 Assistant 消息，支持从 Response 直接构造，或手动传入内容与工具调用"""
    # 如果第一个参数是 Response 对象，自动解包
    if (
        content is not None
        and not isinstance(content, str)
        and hasattr(content, "tool_calls")
    ):
        # 这里的 hasattr 检查是为了避免循环导入或处理 Response 类型的鸭子类型
        resp = content
        content = resp.text
        actions = actions or resp.tool_calls
        reasoning = reasoning or resp.reasoning
...
class Response(BaseModel):
    ...
    def reasoning(self) -> Optional[str]:
        return (
            self.output.content.reasoning
            if self.output and self.output.content
            else None
        )
