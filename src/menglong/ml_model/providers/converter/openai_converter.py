...
class OpenAIConverter(BaseConverter):
    ...
    def convert_request(messages):
        """将消息转换为OpenAI兼容格式。"""
        logger = get_logger("converter.openai")

        # 验证输入
        messages = BaseConverter._validate_input_messages(
            messages, "OpenAI.convert_request"
        )
        ...
        try:
            for index, message in enumerate(messages):
                logger.debug(f"处理消息 {index}: {type(message).__name__}")
            ...
            logger.info(
                f"OpenAI 请求转换完成: {len(messages)} -> {len(format_messages)} 条消息"
            )
    ...
    def convert_tools(tools):
        ...
        # 验证输入
        tools = BaseConverter._validate_input_tools(tools, "OpenAI.convert_tools")
        ...
        try:
            for index, tool in enumerate(tools):
                logger.debug(
                    f"转换工具 {index}: {getattr(tool, '_tool_info', {}).name if hasattr(tool, '_tool_info') else '未知'}"
                )

                # 检查是否是函数对象（有 _tool_info 属性）
                if hasattr(tool, "_tool_info"):
                    ...
                    error_msg = f"不支持的工具格式: {type(tool)}"
                    logger.error(error_msg)
                    raise ValueError(error_msg)

                formatted_tools.append(formatted_tool)

            logger.info(f"OpenAI 工具转换完成: {len(tools)} -> {len(formatted_tools)}")
            ...
            print_json(
                {
                    ...
                    "processed_count": len(formatted_tools),
                    "error_trace": traceback.format_exc(),
                },
                title="OpenAI 工具转换错误详情",
            )
            raise
    ...
    def normalize_response(response):
        ...
        try:
            logger.debug(f"开始处理 OpenAI 响应，响应类型: {type(response).__name__}")

            # 检查响应结构
            if not hasattr(response, "choices") or not response.choices:
                error_msg = "OpenAI 响应缺少 choices 字段或为空"
                logger.error(error_msg)
                raise ValueError(error_msg)
            ...
            if (
                ...
                logger.debug("响应包含推理内容")
            else:
                content.text = response_choices.message.content
                logger.debug("响应仅包含文本内容")
    ...
    def normalize_stream_response(response_stream):
        ...
        try:
            for chunk in response_stream:
                chunk_count += 1
                logger.debug(f"处理流式响应块 {chunk_count}")
            ...
            logger.info(f"OpenAI 流式响应处理完成，共处理 {chunk_count} 个响应块")
